...
export function Dashboard({ onNewGame, onOpenGame, onGoto }: { onNewGame: () => void; onOpenGame: (id: string) => void; onGoto: (tab: string) => void }) {
  ...
  const liveCard = liveGame ? (
      <button className="live-card" onClick={() => onOpenGame(liveGame.id)}>
        <div className="row between" style={{ width: '100%' }}>
          <div className="live-badge"><span className="live-dot" /> שולחן מתנהל עכשיו</div>
  ...
  // שולחן שמתנהל עכשיו מוצג בנפרד ואינו נחשב שולחן שנרשם
  ...
  if (closedGames.length === 0) {
    return (
      ...
      {pendingBanner}
        ...
        <Empty
          icon="🃏"
          title="עוד לא נרשם אף שולחן"
          text="הוסיפו את השולחן הראשון — כמה כל אחד נכנס, כמה יצא, והאפליקציה תחשב מי מעביר למי."
          action={
            <button className="btn btn-primary" onClick={onNewGame}>+ שולחן חדש</button>
          }
        />
        <div className="card" style={{ marginTop: 16 }}>
          ...
          <ol className="muted" style={{ fontSize: 13.5, lineHeight: 2, paddingInlineStart: 20, margin: 0 }}>
            <li>מוסיפים את השחקנים ואת סכום הכניסה של השולחן.</li>
            <li>בסוף השולחן מזינים כמה כל אחד יצא עם — הקופה חייבת להתאזן.</li>
            <li>מקבלים רשימת העברות מינימלית, מעתיקים לוואטסאפ, וזהו.</li>
            <li>כל שולחן נשמר ומזין את לוח המובילים, התארים והגרפים.</li>
          </ol>
        </div>
      </div>
    );
  ...
  return (
    <div className="fade-in">
      <div className="section-head">
        <div>
          <h1>{club?.name ?? 'הקלאב'}</h1>
          <p>{closedGames.length} שולחנות · {activeStats.length} שחקנים · {money(totalPot)} עברו על השולחן</p>
        </div>
      ...
      <div className="stat-grid" style={{ marginBottom: 14 }}>
        <Stat label="שולחנות שנרשמו" value={closedGames.length} sub={stats.summaries[0] ? `מאז ${formatDate(stats.summaries[0].game.date)}` : undefined} />
        <Stat label="סה״כ קופה" value={money(totalPot)} sub={`ממוצע ${money(round2(totalPot / Math.max(1, closedGames.length)))} לשולחן`} tone="gold" />
        <Stat
          label="המוביל"
      ...
      {lastSummary && (
        <div className="card">
          ...
          <div className="row between" style={{ marginBottom: 12 }}>
            ...
            <button className="btn btn-sm" onClick={() => onOpenGame(lastSummary.game.id)}>לשולחן המלא →</button>
          </div>
          {lastTransfers.length === 0 ? (
            <p className="muted" style={{ fontSize: 13.5 }}>אין העברות פתוחות בשולחן הזה.</p>
          ) : (
            <div>
          ...
          <div className={`lb-row${i === 0 ? ' top' : ''}`} key={s.player.id} onClick={() => onGoto('leaderboard')}>
            ...
            <div className="lb-main">
              ...
              <div style={{ minWidth: 0 }}>
                ...
                <div className="lb-sub">{s.games} שולחנות · ממוצע {signedMoney(s.avgNet)}</div>
              </div>
            </div>
      ...
      <div className="card">
        <div className="card-title">
          <h2>📅 שולחנות אחרונים</h2>
          <button className="btn btn-sm btn-ghost" onClick={() => onGoto('games')}>כל השולחנות →</button>
        </div>
        {recent.map((g) => {
